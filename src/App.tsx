...
import { ArrowRight, BarChart3, CheckCircle2, ExternalLink, HeartPulse, Search, ShieldCheck, Sparkles, Target, TrendingUp, Video } from 'lucide-react';
...
const fallbackData: HotspotData = {
  ...
  hotspots: [
    { tag: '儿科', title: '儿童呼吸道感染进入高关注期，适合做家庭护理和就诊判断选题', time: '2 小时前', sourceName: '示例选题池', sourceUrl: '', summary: '围绕近期儿童呼吸道感染关注度上升，适合整理家庭观察、就医判断和护理误区。', angle: '家长如何判断居家观察还是就医', score: 92, risk: '需避免替代医生诊断' },
    { tag: '益生菌', title: '儿童益生菌使用场景升温，适合拆成科普边界与常见误区', time: '4 小时前', sourceName: '示例选题池', sourceUrl: '', summary: '适合用短视频解释益生菌适用边界、常见误区，以及哪些情况需要咨询医生。', angle: '益生菌不是万能补充剂', score: 86, risk: '需标注适用边界和咨询医生' },
    { tag: '保健', title: '睡眠、运动、营养管理搜索热度上升，可做系列短视频', time: '6 小时前', sourceName: '示例选题池', sourceUrl: '', summary: '可拆成一组家庭可执行清单，帮助家长快速建立儿童日常保健认知。', angle: '一周儿童健康习惯清单', score: 82, risk: '避免制造焦虑' },
    { tag: '疫苗', title: '校园健康通知与感染预防相关内容适合做政策提醒类选题', time: '8 小时前', sourceName: '示例选题池', sourceUrl: '', summary: '适合跟进学校通知、疾控提醒和感染预防信息，做成家长提醒类内容。', angle: '开学季感染预防提醒', score: 80, risk: '需引用权威机构通知' },
    { tag: '发育', title: '身高体重、视力和口腔管理长期稳定，可沉淀为常青内容', time: '今日早些时候', sourceName: '示例选题池', sourceUrl: '', summary: '适合做长期内容池，按年龄段拆解筛查重点和家庭行动建议。', angle: '不同年龄段的筛查重点', score: 78, risk: '避免单一指标判断发育情况' }
  ],
...
function App() {
  ...
  const hotspots = data.hotspots.slice(0, 12);
  const selectedHotspot = hotspots[selectedIndex] || hotspots[0] || fallbackData.hotspots[0];
    ...
    <main className="min-h-screen bg-[#fbfbf7] text-[#18221d]">
      <header className="sticky top-0 z-10 border-b border-[#e6ebe4] bg-[#fbfbf7]/92 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
          ...
          <a className="inline-flex size-9 items-center justify-center rounded-md border border-[#d7dfd8] bg-white text-[#18221d] transition hover:border-[#9fb1a6]" href="#recent" aria-label="搜索内容"><Search size={17} aria-hidden="true" /></a>
        </div>
      </header>
        ...
        <aside className="rounded-lg border border-[#e1e8e2] bg-white/80 p-5">
          ...
          <div className="space-y-2">
            {hotspots.map((item, index) => {
              ...
              return (
                <button className={`w-full rounded-lg border p-3 text-left transition ${selected ? 'border-[#176f5d] bg-[#f2f8f4]' : 'border-[#edf1ec] bg-white hover:border-[#b7c8bd] hover:bg-[#fbfdf9]'}`} key={`${item.title}-${index}`} onClick={() => setSelectedIndex(index)} type="button">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <div className="rounded-lg border border-[#edf1ec] bg-[#fbfdf9] p-4">
              ...
              {selectedHotspot.sourceUrl ? (
                <a className="mt-3 inline-flex items-center gap-2 rounded-md bg-[#18221d] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#2b3831]" href={selectedHotspot.sourceUrl} rel="noreferrer" target="_blank">打开新闻原文<ExternalLink size={15} aria-hidden="true" /></a>
              ) : (
